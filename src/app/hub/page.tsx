...
import { ArrowUpRight, Copy, Check, Download, Search } from "lucide-react";
...
export default function HubPage() {
  ...
  return <div className="flex min-h-screen flex-col"><SiteHeader rightContent={<a href="https://github.com/The-Conos-Project/ctf-challenges" target="_blank" rel="noreferrer" className="text-sm font-medium text-muted-foreground hover:text-foreground">GitHub repository</a>} />
    <main className="flex-1 px-4 py-12 sm:py-16"><div className="mx-auto max-w-[1000px] space-y-9"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="mb-2 text-sm font-medium text-muted-foreground">Live repository catalogue</p><h1 className="text-4xl font-semibold tracking-tight">Challenge Hub</h1><p className="mt-3 max-w-2xl text-muted-foreground">New Markdown guides and challenge archives appear here automatically. No website redeploy is required.</p></div></div>
      <div className="w-full max-w-[1000px]"><div className="flex gap-2"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleSearch(); }} placeholder="Search challenges" className="w-full rounded-xl border border-border bg-card py-3 pl-10 pr-4 outline-none focus:ring-2 focus:ring-ring" /><button onClick={handleSearch} className="rounded-xl border border-border bg-card px-4 py-2 text-sm font-medium hover:bg-muted transition-colors whitespace-nowrap">Search</button></div></div>
