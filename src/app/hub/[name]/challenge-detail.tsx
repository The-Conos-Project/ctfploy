...
export default function ChallengeDetail({ slug }: { slug: string }) {
  ...
  return <div className="flex min-h-screen flex-col"><SiteHeader badge="Hub" rightContent={<Link href="/hub" className="inline-flex items-center gap-1 text-sm font-medium text-muted-foreground hover:text-foreground"><ArrowLeft className="h-4 w-4" />All challenges</Link>} /><main className="flex-1 px-4 py-12"><div className="mx-auto max-w-4xl">{!challenge && !error && <p className="py-24 text-center text-muted-foreground">Loading challenge guide…</p>}{error && <div className="py-24 text-center"><p className="text-destructive">{error}</p><button className="mt-5 inline-flex items-center gap-2 rounded-lg border border-border px-4 py-2" onClick={() => void load()}><RefreshCw className="h-4 w-4" />Try again</button></div>}{challenge && <ChallengeView challenge={challenge as Required<Challenge>} />}</div></main><Footer /></div>;
}
